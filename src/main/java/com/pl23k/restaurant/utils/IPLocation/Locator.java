...
public final class Locator implements ILocator {
    ...
    public static Locator sLocator = null;

    public static boolean init(String dataFile){
        boolean ret = false;
        if(null == dataFile || dataFile.length() < 1){
            return ret;
        }

        try {
            sLocator = Locator.loadFromLocal(dataFile);
            ret = true;
        }catch (IOException e){
            e.printStackTrace();
            sLocator = null;
     ...
     * @return
     */
    public static String getFullAddr(String ip){
        String addr;
        if(sLocator != null){
            try {
                addr = sLocator.find(ip).toString();
            }catch (Exception e){
                addr = "IP地址错误";
            }
        }else{
